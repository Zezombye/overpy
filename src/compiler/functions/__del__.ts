...
astParsingFunctions.__del__ = function (content, compiler) {
    if (content.args[0].name !== "__valueInArray__") {
        compiler.error("Expected an array access for the 'del' operator, but got " + functionNameToString(content.args[0]), content.args[0].fileStack);
    }
    content = compiler.Ast("__modifyVar__", [content.args[0].args[0], compiler.Ast("__removeFromArrayByIndex__", [], [], "__Operation__"), content.args[0].args[1]]);
    ...
    if (content.args[0].name === "__valueInArray__" && content.args[0].args[0].name === "__valueInArray__") {
        if (content.args[0].args[0].args[0].name === "__valueInArray__") {
            if (content.args[0].args[0].args[0].args[0].name === "__valueInArray__") {
                compiler.error("Cannot delete index of 4d array");
            }
            //this hurts my brain
            ...
            if (compiler.astContainsRandom(index3) || compiler.astContainsRandom(index2) || compiler.astContainsRandom(content.args[0])) {
                compiler.error("Cannot delete from 3d array with a random index");
            }

            return compiler.Ast("__assignTo__", [content.args[0], compiler.parseOpyMacro(`(
            ...
            if (compiler.astContainsRandom(index2) || compiler.astContainsRandom(content.args[0])) {
                compiler.error("Cannot delete from 2d array with a random index");
            }

            return compiler.Ast("__assignTo__", [content.args[0], compiler.parseOpyMacro("$arrayIdx1.slice(0, $idx2).concat([[elem for elem, i in $arrayIdx1[$idx2] if i != $value]]).concat($arrayIdx1.slice($idx2+1))", ["$arrayIdx1", "$idx2", "$value"], [content.args[0], index2, value])]);
        }
    }
    return content;
};
