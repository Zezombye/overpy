...
astParsingFunctions.__format__ = function (content) {
  ...
  if (
    enableOptimization &&
    content.args[0].type === "LocalizedStringLiteral" &&
    [
      "",
      "*",
      "----------",
...
function parseCustomString(str: Ast, formatArgs: Ast[]) {
  ...
  for (var key of Object.keys(numberMapping)) {
    if (formatArgs[+key]) {
      ...
      error(
        "Too few arguments in format() function: expected " +
          (+key + 1) +
          " but found " +
          formatArgs.length,
      );
  ...
  if (isFullwidth && containsNonFullwidthChar) {
    warn(
      "w_not_total_fullwidth",
      "Could not fully convert this string to fullwidth characters",
    );
  }
  if (isBigLetters && !isConvertedToBigLetters) {
    error(
      "Could not convert the string to big letters. The string must have one of the following chars: '" +
        Object.keys(bigLettersMappings).join("") +
        "'",
    );
  }
...
function parseStringTokens(
  tokens: ({ text: string; type: "string" } | { index: number; type: "arg" })[],
  args: Ast[],
) {
  var result = "";
  var resultArgs: Ast[] = [];
  ...
  if (
    ...
    getUtf8Length(tokens[0].text) <= 128
  ) {
    ...
    for (var i = 0; i < tokens.length; i++) {
      ...
      if (
        (token.type === "string" &&
          stringLength + getUtf8Length(token.text) >
            128 - (i === tokens.length - 1 ? 0 : "{0}".length)) ||
        (token.type === "arg" &&
        ...
        if (
          token.type === "string" &&
          (stringLength + getUtf8Length(token.text) > 128 || tokens.length > i)
        ) {
          ...
          for (
            var j = 0;
            stringLength + tokenSliceLength < 128 - "{0}".length * 2;
            j++
          ) {
        ...
        if (splitString) {
          ...
          if (currentNbIndex > 2) {
            error(
              "Custom string parser returned '{" +
                currentNbIndex +
                "}', please report to Zezombye",
            );
          }
          resultArgs.push(
            parseStringTokens(tokens.slice(i, tokens.length), args),
          );
          break;
        }
      }
      ...
      if (token.type === "string") {
        ...
        if (
          numbersEncountered.length >= 2 &&
          (numbers.length > 3 ||
            (i < tokens.length - 1 &&
              stringLength +
                (tokens[i + 1].type === "string"
                  ? getUtf8Length((tokens[i + 1] as { text: string }).text) +
                    "{0}".length
                  : "{0}".length) >
                128))
        ) {
  ...
  if (resultArgs.length !== 3) {
    error(
      "Custom string parser broke (string args length is " +
        resultArgs.length +
        "), please report to Zezombye",
    );
  }
  ...
  return new Ast(
    "__customString__",
    [new Ast(result, [], [], "CustomStringLiteral")].concat(resultArgs),
  );
}
...
function parseLocalizedString(content: Ast, formatArgs: Ast[]) {
  if (formatArgs.length > 3) {
    error(
      "A localized string cannot have more than 3 arguments in the 'format' function",
    );
  }
