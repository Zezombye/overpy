...
astParsingFunctions[".append"] = function (content) {
    ...
    if (!astContainsRandom(content.args[0])) {
        return new Ast("__assignTo__", [content.args[0], new Ast(".concat", [content.args[0], content.args[1]])]);
    }
    return new Ast("__modifyVar__", [content.args[0], new Ast("__appendToArray__", [], [], "__Operation__"), content.args[1]]);
};
