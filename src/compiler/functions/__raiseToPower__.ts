  ...
  if (enableOptimization) {
    ...
    if (
      ...
      return getAstForNumber(
        Math.pow(
          content.args[0].args[0].numValue,
          content.args[1].args[0].numValue,
        ),
      );
    }
    ...
    if (
      content.args[0].name === "__number__" &&
      content.args[0].args[0].numValue === 1
    ) {
      return content.args[0];
    }
    ...
    //negative number ** A -> 0
    if (
      content.args[0].type !== "Value" &&
      isTypeSuitable("signed float", content.args[0].type)
    ) {
      return getAstFor0();
    }
  }
