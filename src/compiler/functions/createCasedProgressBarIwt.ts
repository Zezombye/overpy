...
astParsingFunctions.createCasedProgressBarIwt = function (content, compiler) {
    if (content.args[0].name !== "__number__") {
        compiler.error("First argument of createCasedProgressBarIwt must be a literal number", content.args[0].fileStack);
    }
    if (content.args[0].args[0].numValue < 2 || content.args[0].args[0].numValue > 6) {
        compiler.error("Number of texts must be between 2 and 6", content.args[0].fileStack);
    }
    if (content.args[2].name !== "__customString__") {
        compiler.error("Text must be a literal custom string", content.args[2].fileStack);
    }
    for (let arg of content.args) {
        if (compiler.astContainsRandom(arg)) {
            compiler.error("Cannot use random functions in createCasedProgressBarIwt", arg.fileStack);
        }
    }
    ...
    for (let i = 0; i < nbTexts; i++) {
        iwts.push(
            compiler.Ast("createProgressBarInWorldText", [
                content.args[1], //visibility
                compiler.getAstForNumber(i), //percentage
                compiler.getAstForCustomString(casedTexts[i], content.args[2].args.slice(1)), //text
                content.args[3], //position
                content.args[4], //scale
                content.args[5], //clipping
                compiler.getAstForColorWhite(), //progress bar color
