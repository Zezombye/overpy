...
astParsingFunctions.continue = function (content) {
  ...
  while (innermostStructure.parent !== undefined) {
    if (
      ["__while__", "__for__", "__doWhile__"].includes(innermostStructure.name)
    ) {
      break;
    } else {
  ...
  if (innermostStructure.name === "__doWhile__") {
    ...
    innermostStructure.name === "__while__" ||
    innermostStructure.name === "__for__"
  ) {
    ...
    innermostStructure.children.splice(
      innermostStructure.children.length,
      0,
      label,
    );
    ...
    return new Ast("__skip__", [
      new Ast("__distanceTo__", [new Ast(labelName, [], [], "Label")]),
    ]);
  } else {
    warn(
      "w_continue_outside_loop",
      "Found 'continue' instruction, but not within a loop",
    );
