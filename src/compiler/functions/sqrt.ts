...
import { areAstsAlwaysEqual, Ast, astParsingFunctions } from "../../utils/ast";

astParsingFunctions.sqrt = function (content, compiler) {
    if (compiler.enableOptimization) {
        if (content.args[0].name === "__number__") {
            ...
            return compiler.getAstForNumber(Math.sqrt(content.args[0].args[0].numValue) || 0);
        }
        if (content.args[0].name === "dotProduct" && areAstsAlwaysEqual(content.args[0].args[0], content.args[0].args[1])) {
            ...
            return compiler.Ast("magnitude", [content.args[0].args[0]]);
        }
    }
