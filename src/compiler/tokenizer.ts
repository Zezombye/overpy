...
import {
  BaseNormalFileStackMember,
  FileStackMember,
  FunctionMacroData,
  MacroData,
  MacroFileStackMember,
...
export async function tokenize(content: string): Promise<LogicalLine[]> {
  ...
  function moveCursor(amount: number) {
    for (let j = 0; j < amount; j++) {
      ...
      if (currentFile.fileStackMemberType === "webUI") {
        error(
          "WebUI file stack member not supported in this context (currentFile)",
        );
      }
      if (!currentFile.staticMember && currentFile.remainingChars > 0) {
        ...
        if (currentFile.remainingChars === 0) {
          ...
          if (nextFile.fileStackMemberType === "webUI") {
            error(
              "WebUI file stack member not supported in this context (nextFile)",
            );
          }
          ...
          if (!nextFile.staticMember) {
            (nextFile as MacroFileStackMember).remainingChars -=
              currentFile.callNbChars;
          }
          fileStack.pop();
        }
      }
  ...
  async function parsePreprocessingDirective(content: string) {
    ...
    if (
      ...
      content.startsWith("#!defineMember ")
    ) {
      macros.push(
        await parseMacro({
      ...
      if (!(addedExtension in customGameSettingsSchema.extensions.values)) {
        error(
          "Unknown extension '" +
            addedExtension +
            "', valid ones are: " +
            Object.keys(customGameSettingsSchema.extensions.values).join(", "),
        );
      }
  ...
  for (i = 0; i < content.length; moveCursor(1)) {
    ...
    if (content.startsWith("#!", i)) {
      ...
      if (preprocessingDirectiveContent.startsWith("#!include ")) {
        ...
        let paths = await getFilePaths(
          preprocessingDirectiveContent.substring(space),
        );
    ...
    if (content[i] === "#") {
      ...
      if (!foundEndOfComment) {
        error(
          "Multiline comment isn't terminated (found end of file while searching for end of comment)",
        );
      }
      j += "*/".length;
      moveCursor(j - i - 1);
      ...
      if (!foundEndOfString) {
        error(
          "String isn't terminated (found end of file while searching for end of string)",
        );
      }
      ...
      if (j > i) {
        ...
        for (var k = 0; k < macros.length; k++) {
          if (content.substring(i, j) === macros[k].name) {
            ...
            if (macros[k].isFunction) {
              ...
              let bracketPos = getBracketPositions(
                content.substring(i),
                true,
                true,
              );
              text = content.substring(i, i + bracketPos[1] + 1);
              let macroArgs = getArgs(
                content.substring(i + bracketPos[0] + 1, i + bracketPos[1]),
              );
              replacement = await resolveMacro(
                macros[k],
                macroArgs,
                currentLine.indentLevel,
              );
            ...
            content =
              ...
              replacement +
              content.substring(i + text.length);
            ...
            addFile(
              ...
              (
                macros[k].fileStack[
                  macros[k].fileStack.length - 1
                ] as BaseNormalFileStackMember
              ).currentLineNb,
            );
        ...
        if (!macroWasFound) {
          ...
          if (
            (content.substring(i, j) === "min" ||
              content.substring(i, j) === "max") &&
            content[i + "min".length] === "="
          ) {
            j++;
          }
          addToken(content.substring(i, j));
      ...
      try {
        ...
        for (let line of stackTrace) {
          ...
          if (encounteredEval) {
            ...
            let lineNb = parseInt(
              line.substring(
                line.substring(0, line.lastIndexOf(":")).lastIndexOf(":") + 1,
                line.lastIndexOf(":"),
              ),
            );
...
async function parseMacro(initialMacroData: {
  fileStack: FileStackMember[];
  content: string;
}): Promise<MacroData> {
  let trimmedMacroContent = initialMacroData.content.substring(
    initialMacroData.content.indexOf(" ") + 1,
  );
  let bracketPos = getBracketPositions(trimmedMacroContent, false, true);
  const isFunctionMacro =
    bracketPos.length > 0 && trimmedMacroContent.indexOf(" ") >= bracketPos[0];
  ...
  let macro: MacroData = {
    ...initialMacroData,
    content: initialMacroData.content.substring(
      initialMacroData.content.indexOf(" ") + 1,
    ),
    isMember: initialMacroData.content.startsWith("#!defineMember "),
    startingCol:
      initialMacroData.content.indexOf(" ") +
      1 +
      (isFunctionMacro
        ? bracketPos[1] +
          1 +
          trimmedMacroContent.substring(bracketPos[1] + 1).search(/\S/) +
          1
        : trimmedMacroContent.indexOf(" ") +
          trimmedMacroContent
            .substring(trimmedMacroContent.indexOf(" "))
            .search(/\S/) +
          1),
    isFunction: isFunctionMacro,
    text: macroText,
    name: isFunctionMacro
      ? trimmedMacroContent.substring(0, bracketPos[0]).trim()
      : macroText,
  ...
  if (!macro.isFunction) {
    ...
    if (reservedNames.includes(macro.name)) {
      warn(
        "w_redefining_keyword",
        "The macro name '" + macro.name + "' is a keyword",
      );
    }
  } else {
    ...
    //Function macro
    functionMacro.args = getArgs(
      functionMacro.content.substring(bracketPos[0] + 1, bracketPos[1]),
    );

    //Test for script macro
    if (functionMacro.replacement.startsWith("__script__(")) {
      functionMacro.isScript = true;
      functionMacro.scriptPath = (
        await getFilePaths(
          functionMacro.replacement.substring(
            "__script__(".length,
            functionMacro.replacement.length - 1,
          ),
        )
      )[0];
    } else {
