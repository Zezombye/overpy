...
export function parseAstRules(rules: Ast[]) {
  ...
  for (var rule of rules) {
    ...
    for (; i < rule.children.length; i++) {
      ...
      if (
        ["@Name", "@Event", "@Team", "@Slot", "@Hero"].includes(
          rule.children[i].name,
        )
      ) {
        const annotationToPropMap: Record<
          string,
        ...
        if (
          rule.children[i].args.length === 3 &&
          (rule.children[i].args[0].name === "Team" ||
            rule.children[i].args[0].name === "Hero") &&
          rule.children[i].args[1].name === "."
        ) {
          rule.children[i].args = [rule.children[i].args[2]];
          rule.children[i].args[0].name =
            rule.children[i].args[0].name.toLowerCase();
        }
        ...
        if (rule.children[i].args.length !== 1) {
          error(
            "Annotation '" +
              ...
              rule.children[i].args.length,
          );
        }
        if (
          annotationToPropMap[rule.children[i].name].prop in rule.ruleAttributes
        ) {
          error(
            "Rule " +
              annotationToPropMap[rule.children[i].name].display +
        ...
        if (rule.children[i].name === "@Name") {
          ...
          rule.ruleAttributes[annotationToPropMap[rule.children[i].name].prop] =
            rule.children[i].args[0].name;
        } else {
          rule.ruleAttributes[annotationToPropMap[rule.children[i].name].prop] =
            rule.children[i].args[0].name;
        ...
        if (rule.children[i].args.length < 1) {
          error(
            "Annotation '" +
              rule.children[i].name +
              "' takes at least 1 argument, received " +
              rule.children[i].args.length,
          );
        }
        ...
        if (rule.children[i].args.length > 1) {
          error(
            "Annotation '@NewPage' takes at most 1 argument, received " +
              rule.children[i].args.length,
          );
        }
        var fillerName = "";
        ...
        while (
          rulesResult.filter((x) => x.name === "__rule__").length % 100 !== 0 ||
          rulesResult.filter((x) => x.name === "__rule__").length === 0
        ) {
    ...
    if (rule.name === "__rule__") {
      ...
      if (rule.ruleAttributes.event === "global") {
        if (
          rule.ruleAttributes.eventTeam !== undefined ||
          rule.ruleAttributes.eventPlayer !== undefined
        ) {
          error(
            "Cannot declare event team or event player with event type '" +
              rule.ruleAttributes.event +
              "'",
          );
        }
      } else {
      ...
      if (rule.ruleAttributes.name === undefined) {
        rule.ruleAttributes.name =
          "Subroutine " + rule.ruleAttributes.subroutineName;
      }
      rule.name = "__rule__";
    ...
    if (rule.ruleAttributes.conditions !== undefined) {
      for (var i = 0; i < rule.ruleAttributes.conditions.length; i++) {
        rule.ruleAttributes.conditions[i] = parseAst(
          rule.ruleAttributes.conditions[i],
        );
        rule.ruleAttributes.conditions[i].comment =
          rule.ruleAttributes.conditionComments[i];
      }
    }
...
export function parseAst(content: Ast) {
  ...
  if (
    ...
    !isTypeSuitable("StringLiteral", content.type)
  ) {
  ...
  if (
    ...
    !["Hero", "Map", "Gamemode", "Team", "Button", "Color"].includes(
      content.type,
    )
  ) {
    if (
  ...
  if (
    ...
    if (
      content.parent &&
      ["__format__", "__customString__", "__localizedString__"].includes(
        content.parent.name,
      ) &&
      content.parent.argIndex === 0
    ) {
      return content;
    } else {
  ...
  if (content.name === "__for__" && content.args.length !== 4) {
    ...
    if (content.args[0].name !== "__arrayContains__") {
      error(
        "Expected the 'in' operator within 'for' directive, but got " +
          functionNameToString(content.args[0]),
      );
    }
    if (content.args[0].args.length !== 2) {
      error(
        "Operator 'in' takes 2 operands, received " +
          content.args[0].args.length,
      );
    }
    if (content.args[0].args[0].name !== "range") {
      error(
        "Expected the 'range' function for the 2nd operand of the 'in' operator, but got " +
          functionNameToString(content.args[0].args[1]),
      );
    }

    if (
      content.args[0].args[0].args.length < 1 ||
      content.args[0].args[0].args.length > 3
    ) {
      error(
        "Function 'range' takes 1 to 3 arguments, received " +
          content.args[0].args[0].args.length,
      );
    }
    ...
    if (content.args.length < 6 || content.args.length > 7) {
      error(
        "Function '" +
          content.name +
          "' takes 6 or 7 arguments, received " +
          content.args.length,
      );
    }
    ...
    if (content.args.length < 10 || content.args.length > 11) {
      error(
        "Function '" +
          content.name +
          "' takes 10 or 11 arguments, received " +
          content.args.length,
      );
    }
    ...
    if (content.args.length < 1 || content.args.length > 2) {
      error(
        "Function '" +
          content.name +
          "' takes 1 or 2 arguments, received " +
          content.args.length,
      );
    }
    if (content.args.length === 1) {
    ...
    if (content.args.length !== 3) {
      error(
        "Function 'rgb' takes 3 arguments, received " + content.args.length,
      );
    }
    ...
    if (content.args.length > 2) {
      error(
        "Function 'wait' takes 0 to 2 arguments, received " +
          content.args.length,
      );
    }
  ...
  if (
    !["__format__", "__array__", "__dict__", "__enumType__"].includes(
      content.name,
    )
  ) {
  ...
  if (content.name === "__format__") {
    if (content.args.length < 1) {
      error(
        "Function '__format__' takes at least 1 argument, received " +
          content.args.length,
      );
    }
    //Check types
    if (
      !isTypeSuitable(
        funcKw[content.name]?.args?.[0].type ?? "",
        content.args[0].type,
      )
    ) {
      warn(
    ...
    for (var i = 1; i < content.args.length; i++) {
      if (
        !isTypeSuitable(
          funcKw[content.name]?.args?.[1].type,
          content.args[i].type,
        )
      ) {
        warn(
    ...
    content.name === "__dict__" ||
    content.name === "__enumType__"
  ) {
    //Check types
    for (var i = 0; i < content.args.length; i++) {
      if (
  ...
  if (
    content.name !== "__rule__" &&
    content.parent !== undefined &&
    content.parent.argIndex !== -1
  ) {
    if (content.parent.name === "__format__" && content.parent.argIndex > 0) {
      content.expectedType =
        funcKw[content.parent.name].args?.[1].type ?? "__INVALID__";
    } else if (
      content.parent.name === "__array__" ||
      content.parent.name === "__dict__" ||
      content.parent.name === "__enumType__"
    ) {
      content.expectedType =
        funcKw[content.parent.name].args?.[0].type ?? "__INVALID__";
    } else if (content.parent.name === "@Condition") {
      content.expectedType = "bool";
    } else if (content.parent.name in funcKw) {
      content.expectedType =
        funcKw[content.parent.name].args?.[content.parent.argIndex].type ??
        "__INVALID__";
    } else {
