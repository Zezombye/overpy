...
export async function compile(
  ...
  if (content.startsWith("#!mainFile ")) {
    let mainFilePath = (
      await getFilePaths(
        content.substring("#!mainFile ".length, content.indexOf("\n")),
      )
    )[0];
  ...
  for (var ext of activatedExtensions) {
    spentExtensionPoints +=
      customGameSettingsSchema.extensions.values[ext].points;
  }
...
function compileRules(astRules: Ast[]) {
  ...
  if (nbElements > ELEMENT_LIMIT) {
    warn(
      "w_element_limit",
      "The gamemode is over the element limit (" +
        nbElements +
        " > " +
        ELEMENT_LIMIT +
        " elements)",
  ...
  if (compiledCustomGameSettings !== "") {
    if (spentExtensionPoints > availableExtensionPoints) {
      warn(
        "w_extension_points",
        "The extension points spent (" +
          spentExtensionPoints +
          ") are over the available points (" +
          availableExtensionPoints +
          ")",
      );
    }
      ...
      if (outputVariables[variable.index] !== undefined) {
        error(
          "Duplicate use of index " +
            ...
            variable.name +
            "' and '" +
            outputVariables[variable.index] +
            "'",
        );
      }
...
function generateSubroutinesField() {
  ...
  for (let subroutine of subroutines) {
    ...
    if (outputSubroutines[subroutine.index] !== undefined) {
      error(
        "Duplicate use of index " +
          ...
          subroutine.name +
          "' and '" +
          outputSubroutines[subroutine.index] +
          "'",
      );
    }
    ...
    if (subroutine.index === undefined || subroutine.index === -1) {
      ...
      if (
        isNaN(subroutine.index) ||
        subroutine.index >= 128 ||
        subroutine.index < 0
      ) {
        error(
          "Invalid index '" +
...
export function compileCustomGameSettings(
  customGameSettings: Record<string, any>,
) {
  ...
  for (var key of Object.keys(customGameSettings)) {
    if (key === "main" || key === "lobby") {
      ...
      if (
        key === "lobby" &&
        customGameSettings["lobby"].dataCenterPreference === "bestAvailable"
      ) {
        delete customGameSettings["lobby"].dataCenterPreference;
      }
      result[tows(key, customGameSettingsSchema)] =
      ...
      if (key === "lobby") {
        ...
        if ("team1Slots" in customGameSettings["lobby"]) {
          ...
          for (var gamemode in customGameSettings.gamemodes) {
            ...
            if ("defaultTeam1Slots" in gamemodeEntry) {
              maxTeam1Slots = Math.max(
                maxTeam1Slots,
                gamemodeEntry.defaultTeam1Slots as number,
              );
            }
          }
        }
        ...
        if ("team2Slots" in customGameSettings["lobby"]) {
          ...
          for (var gamemode in customGameSettings.gamemodes) {
            ...
            if ("defaultTeam2Slots" in gamemodeEntry) {
              maxTeam2Slots = Math.max(
                maxTeam2Slots,
                gamemodeEntry.defaultTeam2Slots as number,
              );
            }
          }
        }
        ...
        if ("ffaSlots" in customGameSettings["lobby"]) {
          ...
          for (var gamemode in customGameSettings.gamemodes) {
            ...
            if ("defaultFfaSlots" in gamemodeEntry) {
              maxFfaSlots = Math.max(
                maxFfaSlots,
                gamemodeEntry.defaultFfaSlots as number,
              );
            }
          }
        }
        ...
        if (maxSlots > 12) {
          error(
            "The maximum number of slots cannot be over 12 (currently " +
              maxSlots +
              ")",
          );
        }
      ...
      for (var gamemode of Object.keys(customGameSettings.gamemodes)) {
        ...
        if (
          "enabled" in customGameSettings.gamemodes[gamemode] &&
          customGameSettings.gamemodes[gamemode].enabled === false
        ) {
        ...
        if (
          "enabledMaps" in customGameSettings.gamemodes[gamemode] ||
          "disabledMaps" in customGameSettings.gamemodes[gamemode]
        ) {
          if (
            "enabledMaps" in customGameSettings.gamemodes[gamemode] &&
            "disabledMaps" in customGameSettings.gamemodes[gamemode]
            ...
            error(
              "Cannot have both 'enabledMaps' and 'disabledMaps' in gamemode '" +
                gamemode +
                "'",
            );
          }
          ...
          for (var map of customGameSettings.gamemodes[gamemode][mapsKey]) {
            if (typeof map === "object" && !Array.isArray(map)) {
              ...
              result[wsGamemodes][wsGamemode][wsMapsKey].push(
                tows(mapName, mapKw) + " " + variants.join(" "),
              );
            } else {
              if (!(map in mapKw)) {
      ...
      for (var team of Object.keys(customGameSettings.heroes)) {
        ...
        var wsHeroesKeyObj = [];
        if (
          "enabledHeroes" in customGameSettings.heroes[team] ||
          "disabledHeroes" in customGameSettings.heroes[team]
        ) {
          if (
            "enabledHeroes" in customGameSettings.heroes[team] &&
            "disabledHeroes" in customGameSettings.heroes[team]
            ...
            error(
              "Cannot have both 'enabledHeroes' and 'disabledHeroes' in team '" +
                team +
                "'",
            );
          }
      ...
      for (var workshopSetting of Object.keys(customGameSettings.workshop)) {
        if (customGameSettings.workshop[workshopSetting] === true) {
          result[wsWorkshop][workshopSetting] = tows(
            "__on__",
            customGameSettingsKw,
          );
          ...
          Array.isArray(customGameSettings.workshop[workshopSetting])
        ) {
          //Enum value
          if (customGameSettings.workshop[workshopSetting].length !== 1) {
            error(
              "Invalid value '" +
                customGameSettings.workshop[workshopSetting] +
                "' for workshop setting '" +
                workshopSetting +
                "', must be of length 1",
            );
          }
          result[wsWorkshop][workshopSetting] =
            "[" + customGameSettings.workshop[workshopSetting] + "]";
  ...
  if (activatedExtensions.length > 0) {
    ...
    result[tows("extensions", customGameSettingsSchema)] =
      activatedExtensions.map((x) =>
        tows(x, customGameSettingsSchema.extensions.values),
      );
  }
    ...
    for (var key of Object.keys(obj)) {
      if (obj[key].constructor === Array) {
        result +=
          ...
          "{\n" +
          obj[key]
            .map((x: any) => tabLevel(nbTabs + 1, true) + x + "\n")
            .join("");
        result += tabLevel(nbTabs, true) + "}\n";
      } else if (typeof obj[key] === "object" && obj[key] !== null) {
        result +=
          tabLevel(nbTabs, true) + key + deserializeObject(obj[key]) + "\n";
      } else {
  ...
  setCompiledCustomGameSettings(
    tows("__settings__", ruleKw) + deserializeObject(result) + "\n",
  );
}
