...
export function astRulesToWs(rules: Ast[]) {
  ...
  for (var rule of rules) {
    ...
    if (rule.ruleAttributes.eventTeam) {
      result +=
        tabLevel(2) + tows(rule.ruleAttributes.eventTeam, eventTeamKw) + ";\n";
    }
    if (rule.ruleAttributes.eventPlayer) {
      result +=
        tabLevel(2) +
        tows(rule.ruleAttributes.eventPlayer, eventPlayerKw) +
        ";\n";
    }
    if (rule.ruleAttributes.subroutineName) {
      result +=
        tabLevel(2) +
        translateSubroutineToWs(rule.ruleAttributes.subroutineName) +
        ";\n";
    }
    result += tabLevel(1) + "}\n";
    ...
    if (
      ...
      rule.ruleAttributes.conditions.length > 0
    ) {
  ...
  if (condition.type === "void") {
    ...
    error(
      "Expected a value, but got " +
        functionNameToString(condition) +
        " which is an action",
    );
  }
  ...
  if (condition.name in funcToOpMapping) {
    ...
    if (enableOptimization && optimizeForSize) {
      for (var i = 0; i <= 1; i++) {
        if (
          ...
          replacementForTeam1 !== "" &&
          condition.args[i].name === "__team__" &&
          condition.args[i].args[0].name === "1"
        ) {
    ...
    for (var i = 0; i < condition.args.length; i++) {
      if (condition.args[i].type === "void") {
        ...
        error(
          "Expected a value, but got " +
            functionNameToString(condition.args[i]) +
            " which is an action",
        );
      }
    }
    ...
    result +=
      " " +
      funcToOpMapping[condition.name as keyof typeof funcToOpMapping] +
      " ";
    ...
    if (condition.name === "__not__") {
      result +=
        tabLevel(2) +
        astToWs(condition.args[0]) +
        " == " +
        tows("false", valueFuncKw) +
        ";\n";
...
export function astActionToWs(action: Ast, nbTabs: number) {
  ...
  if (action.type !== "void") {
    ...
    error(
      "Expected an action, but got " +
        functionNameToString(action) +
        " which is a value",
    );
  }
  let result = "";
...
function astToWs(content: Ast): string {
  ...
  if (enableOptimization && optimizeForSize) {
    ...
    for (var i = 0; i < content.args.length; i++) {
      let argInfo =
        content.name === "__array__"
          ? funcKw[content.name].args?.[0]
          : funcKw[content.name].args?.[i];
      if (argInfo === undefined) {
        error(
          "Could not find info about argument " +
            (content.name === "__array__" ? 0 : i) +
            " of " +
            content.name,
        );
      }
      if (content.args[i].name === "__number__") {
        if (
          argInfo.canReplace0ByFalse &&
          content.args[i].args[0].numValue === 0
        ) {
          content.args[i] = getAstForFalse();
        } else if (
          argInfo.canReplace0ByNull &&
          content.args[i].args[0].numValue === 0
        ) {
          content.args[i] = getAstForNull();
        } else if (
          argInfo.canReplace1ByTrue &&
          content.args[i].args[0].numValue === 1
        ) {
        ...
        argInfo.canReplaceNullVectorByNull &&
        content.args[i].name === "vect" &&
        content.args[i].args[0].name === "__number__" &&
        content.args[i].args[0].args[0].numValue === 0 &&
        content.args[i].args[1].name === "__number__" &&
        content.args[i].args[1].args[0].numValue === 0 &&
        content.args[i].args[2].name === "__number__" &&
        content.args[i].args[2].args[0].numValue === 0
      ) {
        content.args[i] = getAstForNull();
      } else if (
        replacementForTeam1 !== "" &&
        content.args[i].name === "__team__" &&
        content.args[i].args[0].name === "1"
      ) {
  ...
  if (content.name in funcKw && funcKw[content.name].hasLiteralLimit) {
    for (var i = 0; i < content.args.length; i++) {
      ...
      if (
        literalMax > 0 &&
        content.args[i].name === "__number__" &&
        content.args[i].args[0].numValue > literalMax
      ) {
  ...
  if (content.name in equalityFuncToOpMapping) {
    ...
    content.args.splice(
      ...
      0,
      new Ast(
        equalityFuncToOpMapping[
          content.name as keyof typeof equalityFuncToOpMapping
        ],
        [],
        [],
        "__Operator__",
    ...
    content.name === "__assignTo__" ||
    content.name === "__modifyVar__"
  ) {
    if (
      content.name === "__modifyVar__" &&
      enableOptimization &&
      optimizeForSize &&
      content.args[2].name === "__number__"
    ) {
      ...
      if (
        ...
        ["__appendToArray__", "__removeFromArrayByValue__"].includes(
          content.args[1].name,
        )
      ) {
        if (content.args[2].args[0].numValue === 0) {
    ...
    if (content.args[0].name === "__globalVar__") {
      ...
      content.args = [content.args[0].args[0], content.args[0].args[1]].concat(
      ...
      if (content.args[0].args[0].name === "__globalVar__") {
        ...
        if (enableOptimization && optimizeForSize) {
          ...
          if (
            ...
            content.args[1].name === "__number__" &&
            content.args[1].args[0].numValue === 1
          ) {
        ...
        if (enableOptimization && optimizeForSize) {
          if (
            content.args[2].name === "__number__" &&
            content.args[2].args[0].numValue === 0
          ) {
            content.args[2] = getAstForFalse();
          } else if (
            content.args[2].name === "__number__" &&
            content.args[2].args[0].numValue === 1
          ) {
            content.args[2] = getAstForTrue();
          }
        }
        ...
        error(
          "Cannot modify or assign to " +
            functionNameToString(content.args[0].args[0]),
        );
      }
    } else {
      error(
        "Cannot modify or assign to " + functionNameToString(content.args[0]),
      );
    }
    content.name = newName;
  } else if (
    content.name === "__chaseAtRate__" ||
    content.name === "__chaseOverTime__"
  ) {
    var newName =
      content.name === "__chaseAtRate__" ? "AtRate__" : "OverTime__";
    ...
    if (content.args[0].name === "__globalVar__") {
      ...
      error(
        "Expected a variable for 1st argument of " +
          functionNameToString(content) +
          ", but got " +
          functionNameToString(content.args[0]),
      );
    }
    newName = "__chase" + newName;
    content.name = newName;
  } else if (
    ["__getHitPosition__", "__getPlayerHit__", "__getNormal__"].includes(
      content.name,
    )
  ) {
    ...
    content.args = [
      content.args[0],
      new Ast("__roundUp__", [], [], "__Rounding__"),
    ];
    ...
    content.args = [
      content.args[0],
      new Ast("__roundDown__", [], [], "__Rounding__"),
    ];
  } else if (
    ["hudHeader", "hudSubheader", "hudSubtext"].includes(content.name)
  ) {
    ...
    content.args = [
      content.args[0],
      new Ast("__roundToNearest__", [], [], "__Rounding__"),
    ];
  } else if (
    content.name === "RULE_CONDITION" ||
    content.name === "RULE_START"
  ) {
  ...
  if (content.type === "void") {
    ...
    content.type in constantValues
  ) {
    ...
    if (!(content.name in constantValues[content.type])) {
      error(
        "Unknown " +
          content.type.replace("Literal", "").toLowerCase() +
          " '" +
          content.name +
          "'",
      );
  ...
  if (content.args.length > 0) {
    ...
    for (var i = 0; i < content.args.length; i++) {
      ...
      if (content.args[i].type === "void") {
        ...
        error(
          "Expected a value, but got " +
            functionNameToString(content.args[i]) +
            " which is an action",
        );
      }
  ...
  if (
    ...
    funcKw[content.name].canBePutInBoolean === false
  ) {
  ...
  if (content.type === "void" && content.args !== null) {
    ...
    ["__workshopSettingInteger__", "__workshopSettingReal__"].includes(
      content.name,
    )
  ) {
    ...
    ["__workshopSettingToggle__", "__workshopSettingHero__"].includes(
      content.name,
    )
  ) {
