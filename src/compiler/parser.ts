...
export async function parseLines(lines: LogicalLine[]): Promise<Ast[]> {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (
      ...
      currentLine.tokens[currentLine.tokens.length - 1].text.startsWith("#")
    ) {
      currentComments.push(
        currentLine.tokens[currentLine.tokens.length - 1].text.substring(1),
      );
    ...
    if (
      currentLine.tokens[0].text === "globalvar" ||
      currentLine.tokens[0].text === "playervar" ||
      currentLine.tokens[0].text === "subroutine"
    ) {
      ...
      var index: number =
        currentLine.tokens.length > 2
          ? Number.parseInt(currentLine.tokens[2].text)
          : -1;
      ...
      try {
        if (currentLine.tokens.length === 2) {
          ...
          customGameSettings = JSON.parse(
            safeEval("JSON.stringify(" + (await getFileContent(path)) + ")"),
          );
        } else {
          customGameSettings = JSON.parse(
            safeEval(
    ...
    if (currentLine.tokens[0].text.startsWith("@")) {
      if (
        ["@Condition", "@Name", "@NewPage"].includes(currentLine.tokens[0].text)
      ) {
        var currentLineAst = new Ast(
          currentLine.tokens[0].text,
          currentLine.tokens[0].text === "@NewPage" &&
          currentLine.tokens.length === 1
            ? []
            : [parse(currentLine.tokens.slice(1))],
          [],
          "__Annotation__",
        );
      } else {
        var currentLineAst = new Ast(
          ...
          currentLine.tokens
            .slice(1)
            .map((x) => new Ast(x.text, [], [], "__AnnotationArg__")),
          [],
          "__Annotation__",
        );
      }
      ...
      [
        ...
        "case",
        "default",
      ].includes(currentLine.tokens[0].text)
    ) {
      ...
      var funcName =
        tokenToFuncMapping[
          currentLine.tokens[0].text as keyof typeof tokenToFuncMapping
        ];
      var args: string[] | Ast[] = [];
      var children = [];
      ...
      if (funcName === "__rule__") {
        ...
        instructionRuleAttributes.name = unescapeString(
          lineMembers[0][1].text,
          true,
        );
        ...
        if (
          lineMembers[0].length !== 4 ||
          lineMembers[0][2].text !== "(" ||
          lineMembers[0][3].text !== ")"
        ) {
          ...
          for (var subroutine of subroutines) {
            if (subroutine.name === instructionRuleAttributes.subroutineName) {
              if (subroutine.isFromDefStatement) {
                error(
                  "Duplicate definition of subroutine '" +
                    instructionRuleAttributes.subroutineName +
                    "'",
                );
              } else {
                break;
      ...
      for (; j < lines.length; j++) {
        ...
        if (
          lines[j].indentLevel > currentLineIndent &&
          nextIndentLevel !== null &&
          lines[j].indentLevel < nextIndentLevel
        ) {
      ...
      if (funcName === "__doWhile__") {
        ...
        if (j < lines.length && lines[j].tokens[0].text === "while") {
          if (
            lines[j].tokens[lines[j].tokens.length - 1].text.startsWith("#")
          ) {
            currentComments.push(
              lines[j].tokens[lines[j].tokens.length - 1].text.substring(1),
            );
            lines[j].tokens.pop();
          }
      ...
      if (funcName === "__enum__") {
        if (typeof args[0] === "string") {
          error(
            "First argument for enum must be an instance of Ast, but got a string instead.",
          );
        }
        ...
        for (var k = 0; k < childrenLines.length; k++) {
          ...
          if (
            childrenLines[k].tokens[childrenLines[k].tokens.length - 1]
              .text[0] === "#"
          ) {
            childrenLines[k].tokens = childrenLines[k].tokens.slice(
              0,
              childrenLines[k].tokens.length - 1,
            );
          }
          if (
            childrenLines[k].tokens[childrenLines[k].tokens.length - 1].text !==
            ","
          ) {
          ...
          if (assignOperands.length === 1) {
            ...
            if (typeof lastIntValue === "number") {
              enumMembers[args[0].name][childrenLines[k].tokens[0].toString()] =
                getAstForNumber(lastIntValue);
              lastIntValue++;
            } else if (
              lastIntValue instanceof Ast &&
              lastIntValue.name === "__negate__" &&
              lastIntValue.args[0].name === "__number__"
            ) {
              ...
              if (
                astContainsFunctions(
                  ...
                  notConstantFunctions
                    .filter((v) => ("en-US" as OWLanguage) in v)
                    .map((v) => v["en-US"] as string),
                  false,
                )
              ) {
                warn(
                  ...
                  "The value of " +
                    args[0].name +
                    "." +
                    childrenLines[k].tokens[0] +
                    " seems to not be constant; it will be inlined and not stored.",
                );
              }
...
function getOperator(
  ...
  for (var i = start; i !== end; i += step) {
    if (
      ...
      if (
        allowUnaryPlusOrMinus ||
        (i !== 0 &&
          (!Object.keys(operatorPrecedence).includes(tokens[i - 1].text) ||
            (tokens[i - 1].text === "not" && tokens[i].text === "in"))) ||
...
export function parse(content: Token[], kwargs: Record<string, any> = {}): Ast {
  ...
  if (content[0].text === "goto") {
    ...
    if (content.length === 2) {
      ...
      return new Ast("__skip__", [
        new Ast("__distanceTo__", [new Ast(content[1].text, [], [], "Label")]),
      ]);
    } else {
  ...
  if (
    ...
    content[content.length - 2].text === "+"
  ) {
    var op1 = parse(content.slice(0, content.length - 2));
    return new Ast("__assignTo__", [
      op1,
      new Ast("__add__", [op1, getAstFor1()]),
    ]);
  }
  if (
    content.length > 2 &&
    ...
    return new Ast("__assignTo__", [
      op1,
      new Ast("__subtract__", [op1, getAstFor1()]),
    ]);
  }
  ...
  for (
    var precedence = kwargs.minOperatorPrecedence;
    precedence <= operatorPrecedence["**"];
    precedence++
  ) {
    var operatorsToCheck = Object.keys(operatorPrecedence).filter(
      (x) => operatorPrecedence[x] === precedence,
    );
    ...
    var rtlPrecedence =
      precedence === operatorPrecedence["**"] ||
      precedence === operatorPrecedence["if"] ||
      allowUnary === true;

    var operatorCheck = getOperator(
      content,
      operatorsToCheck,
      rtlPrecedence,
      allowUnary,
    ...
    var operands = [
      content.slice(0, operatorCheck.operatorPosition),
      content.slice(operatorCheck.operatorPosition + 1, content.length),
    ];
    ...
    if (operator === "=") {
      ...
      if (
        operands[0].length > 1 &&
        operands[0][operands[0].length - 1].text === "not"
      ) {
      ...
      if (isNotInOperator) {
        return new Ast("__not__", [
          new Ast("__arrayContains__", [array, value]),
        ]);
      } else {
      ...
      return new Ast(
        opToFuncMapping[operator as keyof typeof opToFuncMapping],
        [op1, op2],
      );
    } else if (
      ...
      if (
        ...
        if (
          operands[0].length > 1 &&
          operands[0][operands[0].length - 1].text === "**"
        ) {
  ...
  if (content[content.length - 1].text === "]") {
    ...
    if (bracketPos.length === 2 && bracketPos[0] === 0) {
      ...
      var value = parse(
        content.slice(
          bracketPos[bracketPos.length - 2] + 1,
          content.length - 1,
        ),
      );
  ...
  //Check for strings
  if (
    content[content.length - 1].text.startsWith('"') ||
    content[content.length - 1].text.startsWith("'")
  ) {
          ...
          if (content[0].text === "l") {
            ...
            error(
              "Invalid string modifier '" +
                content[0].text +
                "', valid ones are 'l' (localized), 'b' (big letters), 'p' (plaintext) and 'w' (fullwidth)",
            );
          }
        } else {
  ...
  if (content.length > 1) {
    if (content[1].text === "(") {
      ...
      error(
        "Expected '(' after '" + name + "', but got '" + content[1].text + "'",
      );
    }
  }
  ...
  if (args === null) {
    ...
    if (enumMembers[name]) {
      return new Ast(
        "__enumType__",
        Object.values(enumMembers[name]),
        [],
        "Type",
      );
    } else if (name in builtInEnumNameToAstInfo) {
      const astInfo =
        builtInEnumNameToAstInfo[name as keyof typeof builtInEnumNameToAstInfo];
      const values = Object.keys(constantValues[astInfo.type])
        .filter((key) => key !== "description")
        .map(
          (key) => new Ast(astInfo.name, [new Ast(key, [], [], astInfo.type)]),
        );
    ...
    if (isVarName(name, true)) {
      return new Ast("__globalVar__", [
        new Ast(name, [], [], "GlobalVariable"),
      ]);
    }
    ...
    if (isNumber(name)) {
      ...
      return new Ast(
        "__number__",
        [new Ast(name, [], [], "UnsignedIntLiteral")],
        [],
        "unsigned int",
      );
    }
  ...
  if (name === "async") {
    ...
    return new Ast("__startRule__", [
      new Ast(subroutineArg, [], [], "Subroutine"),
      parse(args[1]),
    ]);
  }
  ...
  if (name === "chase") {
    ...
    if (
      (args[2][0].text !== "rate" && args[2][0].text !== "duration") ||
      args[2][1].text !== "="
    ) {
      error(
        "3rd argument of function 'chase' must be 'rate = xxxx' or 'duration = xxxx'",
      );
    }

    if (
      args[3].length !== 3 ||
      args[3][0].text !== "ChaseReeval" ||
      args[3][1].text !== "."
    ) {
      error(
        "Expected a member of the 'ChaseReeval' enum as 4th argument for function 'chase', but got '" +
          dispTokens(args[3]) +
          "'",
      );
    }
    ...
    return new Ast(funcName, [
      parse(args[0]),
      parse(args[1]),
      parse(args[2].slice(2)),
      parse(args[3]),
    ]);
  }
  ...
  if (name === "raycast") {
    if (args.length === 5) {
      ...
      if (
        args[3].length >= 2 &&
        (args[3][0].text === "exclude" || args[3][1].text === "=")
      ) {
        args[3] = args[3].slice(2);
      }
      if (
        args[4].length >= 2 &&
        (args[4][0].text === "includePlayerObjects" || args[4][1].text === "=")
      ) {
        args[4] = args[4].slice(2);
      }

      return new Ast("raycast", [
        parse(args[0]),
        parse(args[1]),
        parse(args[2]),
        parse(args[3]),
  ...
  if (name === "sorted") {
    ...
    if (args.length === 2) {
      ...
      if (lambdaArgs[0].length === 2) {
        ...
        error(
          "Expected 1 or 3 tokens after 'lambda', but got " +
            (lambdaArgs.length - 1),
        );
      }
      ...
      error(
        "Function 'sorted' takes 1 or 2 arguments, received " + args.length,
      );
    }
  ...
  if (name === "createWorkshopSetting") {
    ...
    return new Ast("createWorkshopSetting", [
      parseType(args[0]),
      ...args.slice(1).map((x) => parse(x)),
    ]);
  }
  ...
  if (args.length === 0) {
    if (isSubroutineName(name)) {
      return new Ast("__callSubroutine__", [
        new Ast(name, [], [], "Subroutine"),
      ]);
    }
  }
...
function parseMember(object: Token[], member: Token[]) {
  ...
  if (member.length > 1) {
    if (member[1].text === "(") {
      ...
      error(
        "Expected '(' after '" + name + "', but got '" + member[1].text + "'",
      );
    }
  }
  ...
  if (args === null) {
    ...
    if (object.length === 1) {
      ...
      if (
        object[0].text in enumMembers &&
        name in enumMembers[object[0].text]
      ) {
      ...
      if (Object.keys(constantValues).includes(object[0].text)) {
        ...
        builtInEnumNameToAstInfo[
          object[0].text as keyof typeof builtInEnumNameToAstInfo
        ]
      ) {
        const astInfo =
          builtInEnumNameToAstInfo[
    ...
    return new Ast("__playerVar__", [
      parse(object),
      new Ast(name, [], [], "PlayerVariable"),
    ]);
  } else {
    if (
      [
      ...
      if (args.length !== 1) {
        error(
          "Function '" + name + "' takes 1 argument, received " + args.length,
        );
      }
      ...
      return new Ast(
        funcToInternalFuncMap[name as keyof typeof funcToInternalFuncMap],
        [parse(object), parse(args[0])],
      );
    } else if (name === "last") {
      if (args.length !== 0) {
        error(
      ...
      return new Ast(
        "__format__",
        [parse(object)].concat(args.map((x) => parse(x))),
      );
    } else if (["getHitPosition", "getNormal", "getPlayerHit"].includes(name)) {
      if (args.length !== 0) {
        error(
          "Function '" + name + "' takes no argument, received " + args.length,
        );
      }
      ...
      if (args.length !== 2) {
        error(
          "Function 'substring' takes 2 arguments, received " + args.length,
        );
      }
      return new Ast("__substring__", [
        parse(object),
        parse(args[0]),
        parse(args[1]),
      ]);
    ...
    if (ifOperands.length === 1) {
      ...
      if (inOperands[0].length === 1) {
        ...
        if (inOperands[0][1].text !== ",") {
          error(
            "Malformed '[x for a, b in z]': expected ',' but found '" +
              inOperands[0][1].text +
              "'",
          );
        }
        ...
        error(
          "Malformed '[x for y in z]': 1st operand of 'in' has length " +
            inOperands[0].length +
            ", expected 1 or 3",
        );
      }
      ...
      return new Ast("__mappedArray__", [
        parse(inOperands[1]),
        parsedMappingFunction,
      ]);
      ...
      if (inOperands[0].length === 1) {
        ...
        if (inOperands[0][1].text !== ",") {
          error(
            "Malformed '[x for a,b in y if z]': expected ',' but found '" +
              inOperands[0][1].text +
              "'",
          );
        }
        ...
        error(
          "Malformed '[x for a,b in y if z]': 1st operand of 'in' has length " +
            inOperands[0].length +
            ", expected 1 or 3",
        );
      }
      ...
      return new Ast("__mappedArray__", [
        new Ast("__filteredArray__", [parse(ifOperands[0]), condition]),
        mappingFunction,
      ]);
    } else {
      error(
        "Expected 0 or 1 'if' after 'in', but found " + (ifOperands.length - 1),
      );
    }
...
function parseDictionary(content: Token[]) {
  ...
  for (var elem of elems) {
    ...
    if (keyValue.length !== 2) {
      error(
        "Expected a value of the form 'key: value' but got '" +
          dispTokens(elem) +
          "'",
      );
    }
    astElems.push(
      new Ast("__dictElem__", [parse(keyValue[0]), parse(keyValue[1])]),
    );
  }
