...
import {
  activatedExtensions,
  astOperatorPrecedence,
  currentArrayElementName,
  currentArrayIndexName,
  currentRuleHasVariableGoto,
  ...
  for (var rule of rules) {
    ...
    if (rule.ruleAttributes.event === "__subroutine__") {
      ...
      decompiledRuleAttributes +=
        tabLevel(nbTabs) +
        "@Name " +
        escapeString(rule.ruleAttributes.name, false) +
        "\n";
    } else {
      decompiledRule +=
        "rule " + escapeString(rule.ruleAttributes.name, false) + ":\n";

      //Decompile the rule attributes
      if (rule.ruleAttributes.event !== "global") {
        decompiledRuleAttributes +=
          tabLevel(nbTabs) + "@Event " + rule.ruleAttributes.event + "\n";
      }
      if (
        rule.ruleAttributes.eventTeam &&
        rule.ruleAttributes.eventTeam !== "all"
      ) {
        decompiledRuleAttributes +=
          tabLevel(nbTabs) + "@Team " + rule.ruleAttributes.eventTeam + "\n";
      }
      if (
        rule.ruleAttributes.eventPlayer &&
...
export function astActionsToOpy(actions: Ast[]): string {
  ...
  for (var i = 0; i < actions.length; i++) {
    ...
    if (
      ["__elif__", "__else__", "__while__", "__for__"].includes(
        actions[i].name,
      ) &&
      !actions[i].isDisabled
    ) {
      ...
      for (var j = i + 1; j < actions.length; j++) {
        ...
        if (
          actions[i].name === "__elif__" &&
          ["__elif__", "__else__"].includes(actions[j].name)
        ) {
          isEndFound = true;
          break;
        }
      ...
      if (
        ...
        if (actions[i].name === "__while__") {
          ...
          actions[i] = new Ast("__if__", [
            new Ast("__or__", [
              new Ast("__and__", [
                new Ast("__greaterThan__", [actions[i].args[2], getAstFor0()]),
    ...
    if (
      ["__if__", "__elif__", "__else__", "__while__", "__for__"].includes(
        actions[i].name,
      )
    ) {
      ...
      if (
        (actions[i].name === "__elif__" || actions[i].name === "__else__") &&
        !actions[i].isDisabled
      ) {
        if (nbTabs > 1) {
          decrementNbTabs();
      ...
      decompiledAction =
        nameToKeywordMapping[
          actions[i].name as keyof typeof nameToKeywordMapping
        ];
      ...
      if (actions[i].name === "__for__") {
        ...
        if (
          !(
            actions[i].args[1].name === "__number__" &&
            actions[i].args[1].args[0].name === "0" &&
            actions[i].args[3].name === "__number__" &&
            actions[i].args[3].args[0].name === "1"
          )
        ) {
      ...
      actions[i].name === "__abortIf__" &&
      !currentRuleHasVariableGoto
    ) {
      decompiledAction +=
        "if " +
        astToOpy(actions[i].args[0]) +
        ":\n" +
        tabLevel(tabLevelForThisAction + 1) +
        "return";
    } else if (
      actions[i].name === "__abortIfConditionIsFalse__" &&
      !currentRuleHasVariableGoto
    ) {
      decompiledAction +=
      ...
      actions[i].name === "__abortIfConditionIsTrue__" &&
      !currentRuleHasVariableGoto
    ) {
      decompiledAction +=
      ...
      decompiledAction =
        astToOpy(actions[i].args[0]) + " = " + astToOpy(actions[i].args[1]);
      ...
      decompiledAction +=
        "chase(" +
        actions[i].args[0].name +
        ", " +
        astToOpy(actions[i].args[1]) +
        ", rate=" +
        astToOpy(actions[i].args[2]) +
        ", " +
        astToOpy(actions[i].args[3]) +
        ")";
      ...
      decompiledAction +=
        ...
        astToOpy(actions[i].args[0]) +
        "." +
        actions[i].args[1].name +
        ", " +
        astToOpy(actions[i].args[2]) +
        ", rate=" +
        astToOpy(actions[i].args[3]) +
        ", " +
        astToOpy(actions[i].args[4]) +
        ")";
      ...
      if (
        actions[i].args[2].name === "null" &&
        actions[i].args[3].name === "null"
      ) {
        decompiledAction +=
          "hudHeader(" +
          [0, 1, 4, 5, 6, 9, 10]
            .map((x) => astToOpy(actions[i].args[x]))
            .join(", ") +
          ")";
      } else if (
        actions[i].args[1].name === "null" &&
        actions[i].args[3].name === "null"
      ) {
        decompiledAction +=
          "hudSubheader(" +
        ...
        decompiledAction +=
          "hudText(" + actions[i].args.map((x) => astToOpy(x)).join(", ") + ")";
      }
    } else if (actions[i].name === "__loop__") {
      decompiledAction += "goto RULE_START";
    } else if (
      actions[i].name === "__loopIf__" &&
      !currentRuleHasVariableGoto
    ) {
      decompiledAction +=
        "if " +
      ...
      actions[i].name === "__loopIfConditionIsFalse__" &&
      !currentRuleHasVariableGoto
    ) {
      decompiledAction +=
        "if not RULE_CONDITION:\n" +
      ...
      actions[i].name === "__loopIfConditionIsTrue__" &&
      !currentRuleHasVariableGoto
    ) {
      decompiledAction +=
        "if RULE_CONDITION:\n" +
      ...
      if (actions[i].args[1].name in funcToOpMapping) {
        decompiledAction +=
          ...
          " " +
          funcToOpMapping[
            actions[i].args[1].name as keyof typeof funcToOpMapping
          ] +
          " " +
          astToOpy(actions[i].args[2]);
      ...
      if (
        astContainsFunctions(
          actions[i].args[0],
          Object.keys(astOperatorPrecedence),
        )
      ) {
        op1 = "(" + op1 + ")";
      }
      ...
      decompiledAction +=
        "stopChasingVariable(" + actions[i].args[0].name + ")";
      ...
      if (
        !(
          actions[i].args[0].name === "__number__" &&
          actions[i].args[0].args[0].name === "0.016" &&
          actions[i].args[1].name === "IGNORE_CONDITION"
        )
      ) {
        decompiledAction += astToOpy(actions[i].args[0]);
      }
      ...
      if (
        ["createEffect", "createBeam", "playEffect"].includes(
          actions[i].name,
        ) &&
        constantValues[actions[i].args[1].type][actions[i].args[1].name]
          .extension
      ) {
        let extensionName = constantValues[actions[i].args[1].type][
          actions[i].args[1].name
        ].extension as string;
      ...
      if (actions[i].name.startsWith("_&")) {
        ...
        if (funcKw[actions[i].name].args !== null) {
          decompiledAction +=
            "(" + actions[i].args.map((x) => astToOpy(x)).join(", ") + ")";
        }
      }
    }
...
export function astToOpy(content: Ast): string {
  ...
  if (
    content.type === "GlobalVariable" ||
    content.type === "PlayerVariable" ||
    content.type === "Subroutine"
  ) {
    return content.name;
  }
  ...
  if (content.name === "__playerVar__") {
    ...
    if (
      astContainsFunctions(content.args[0], Object.keys(astOperatorPrecedence))
    ) {
      result = "(" + result + ")";
    }
  ...
  if (typeof content.type !== "object" && content.type in constantValues) {
    if (
      ...
      ["__ChaseTimeReeval__", "__ChaseRateReeval__"].includes(content.type)
    ) {
  ...
  if (content.name in funcToOpMapping) {
    ...
    if (
      astContainsFunctions(
        content.args[0],
        Object.keys(astOperatorPrecedence).filter(
          (x) =>
            astOperatorPrecedence[x] <
            astOperatorPrecedence[content.name] +
              (content.name === "__raiseToPower__" ? 1 : 0),
        ),
      )
    ) {
      op1 = "(" + op1 + ")";
    ...
    return (
      op1 +
      " " +
      funcToOpMapping[content.name as keyof typeof funcToOpMapping] +
      " " +
      op2
    );
  }
  ...
  if (content.name === "__arrayContains__") {
    ...
    if (
      astContainsFunctions(
        ...
        Object.keys(astOperatorPrecedence).filter(
          (x) =>
            astOperatorPrecedence[x] <= astOperatorPrecedence[content.name],
        ),
      )
    ) {
      op1 = "(" + op1 + ")";
    ...
    if (
      astContainsFunctions(
        content.args[1],
        Object.keys(astOperatorPrecedence).filter(
          (x) => astOperatorPrecedence[x] < astOperatorPrecedence[content.name],
        ),
      )
    ) {
      op2 = "(" + op2 + ")";
  ...
  if (content.name in internalFuncToFuncMap) {
    ...
    return (
      result +
      "." +
      internalFuncToFuncMap[
        content.name as keyof typeof internalFuncToFuncMap
      ] +
      "(" +
      astToOpy(content.args[1]) +
      ")"
    );
  }
  ...
  if (
    ...
    if (
      astContainsFunctions(content.args[1], ["__currentArrayIndex__"]) &&
      !astContainsFunctions(content.args[1], ["__currentArrayElement__"]) &&
      !(
        content.name === "__mappedArray__" &&
        content.args[0].name === "__filteredArray__" &&
        astContainsFunctions(content.args[0].args[1], [
    ...
    if (content.name === "__all__" || content.name === "__any__") {
      ...
      if (content.args[1].name === "__currentArrayElement__") {
        ...
        result +=
          "[" + astToOpy(content.args[1]) + " for " + currentArrayElementName;
      ...
      if (content.args[0].name === "__filteredArray__") {
        result +=
          astToOpy(content.args[0].args[0]) +
          " if " +
          astToOpy(content.args[0].args[1]);
      } else {
        result += opyArray;
      }
      result += "]";
      ...
      result +=
        "[" + currentArrayElementName + " for " + currentArrayElementName;
      if (astContainsFunctions(content.args[1], ["__currentArrayIndex__"])) {
        result += ", " + currentArrayIndexName;
      }
      result += " in ";
  ...
  if (
    ...
    content.name === "__localizedString__"
  ) {
    var formatArgs = [];
    ...
    if (formatArgs.length > 0) {
      result +=
        ".format(" +
        formatArgs
          .map((x) => (x === undefined ? "null" : astToOpy(x)))
          .join(", ") +
        ")";
    }
    return result;
  ...
  if (content.name === "__raycastHitNormal__") {
    return (
      "raycast(" +
      content.args.map((x) => astToOpy(x)).join(", ") +
      ").getNormal()"
    );
  }
  ...
  if (funcKw[content.name].args === null) {
    ...
    return (
      content.name + "(" + content.args.map((x) => astToOpy(x)).join(", ") + ")"
    );
  }
}
