...
export function decompileAllRules(
  content: string,
    ...
    for (var variable of globalVariables) {
      if (defaultVarNames.indexOf(variable.name) !== variable.index) {
        globalVariableDeclarations +=
          "globalvar " + variable.name + " " + variable.index + "\n";
      }
    }
    ...
    for (var variable of playerVariables) {
      if (defaultVarNames.indexOf(variable.name) !== variable.index) {
        playerVariableDeclarations +=
          "playervar " + variable.name + " " + variable.index + "\n";
      }
    }
  ...
  if (subroutines.length > 0) {
    ...
    for (var subroutine of subroutines) {
      if (
        defaultSubroutineNames.indexOf(subroutine.name) !== subroutine.index
      ) {
        subroutineDeclarations +=
          "subroutine " + subroutine.name + " " + subroutine.index + "\n";
      }
    }
...
function decompileAllRulesToAst(content: string): string | [string, Ast[]] {
  ...
  var mapRegex = new RegExp(
    "\\b" + mapConstFunction + "\\(\\s*(?=[&\\-|=\\*,?;\\.:!])",
    "g",
  );
  content = content.replace(
    mapRegex,
    mapConstFunction + "(__removed_from_ow2__)",
  );
  var gamemodeRegex = new RegExp(
    "\\b" + gamemodeConstFunction + "\\(\\s*(?=[&\\-|=\\*,?;\\.:!])",
    "g",
  );
  content = content.replace(
    gamemodeRegex,
    gamemodeConstFunction + "(__removed_from_ow2__)",
  );
  ...
  if (content.startsWith(tows("__settings__", ruleKw))) {
    customGameSettings += decompileCustomGameSettings(
      content.substring(bracketPos[0] + 1, bracketPos[1]),
    );
  ...
  if (content.startsWith(tows("__actions__", ruleKw))) {
    return astActionsToOpy(
      decompileActions(content.substring(bracketPos[0], bracketPos[1] + 1)),
    );
  }
  if (content.startsWith(tows("__conditions__", ruleKw))) {
    return decompileConditions(
      content.substring(bracketPos[0], bracketPos[1] + 1),
    )
      .map((x) => "@Condition " + astToOpy(x))
      .join("\n");
  }
  ...
  for (var i = 0; i < bracketPos.length - 1; i += 4) {
    ...
    astRules.push(
      decompileRuleToAst(
        content.substring(bracketPos[i] + 1, bracketPos[i + 4] + 1),
      ),
    );
  }
...
function decompileCustomGameSettings(content: string) {
  ...
  for (var category of Object.keys(serialized)) {
    ...
    if (opyCategory === "main" || opyCategory === "lobby") {
      result[opyCategory] = decompileCustomGameSettingsDict(
        Object.keys(serialized[category]),
        customGameSettingsSchema[opyCategory].values,
      );
    } else if (opyCategory === "extensions") {
      setActivatedExtensions(
        Object.keys(serialized[category]).map((ext) =>
          topy(ext, customGameSettingsSchema.extensions.values),
        ),
      );
      ...
      for (var gamemode of Object.keys(serialized[category])) {
        ...
        if (gamemode.startsWith(wsDisabled)) {
          ...
          var opyGamemode = topy(
            gamemode.substring(wsDisabled.length),
            customGameSettingsSchema.gamemodes.values,
          );
        } else {
          var opyGamemode = topy(
            gamemode,
        ...
        if (serialized[category][gamemode] !== null) {
          for (var property of Object.keys(serialized[category][gamemode])) {
            if (serialized[category][gamemode][property] === null) {
              ...
              var opyPropName = topy(
                property,
                customGameSettingsSchema.gamemodes.values.general.values,
              );
              result[opyCategory][opyGamemode][opyPropName] = [];
              for (var map of Object.keys(
                serialized[category][gamemode][property],
              )) {
                ...
                if (map.includes("972777")) {
                  var variants = [...map.matchAll(/\b972777\d+\b/g)].map(
                    (x) => x[0],
                  );
                  ...
                  for (var variant of variants) {
                    var variantName = Object.keys(
                      mapKw[mapName].variants as Record<string, string>,
                    ).filter(
                      (x) =>
                        (mapKw[mapName].variants as Record<string, string>)[
                          x
                  ...
                  result[opyCategory][opyGamemode][opyPropName].push(
                    topy(map, mapKw),
                  );
                }
              }
        ...
        Object.assign(
          ...
          decompileCustomGameSettingsDict(
            dict,
            customGameSettingsSchema[opyCategory].values[opyGamemode].values,
          ),
        );
      }
      ...
      for (var team of Object.keys(serialized[category])) {
          ...
          if (serialized[category][team][property] === null) {
            ...
            if (
              property ===
                tows(
                  "disabledHeroes",
                  customGameSettingsSchema.heroes.values,
                ) ||
              property ===
                tows("enabledHeroes", customGameSettingsSchema.heroes.values)
            ) {
              var opyPropName = topy(
                property,
                customGameSettingsSchema.heroes.values,
              ...
              for (var hero of Object.keys(
                serialized[category][team][property],
              )) {
                result[opyCategory][opyTeam][opyPropName].push(
                  topy(hero, heroKw),
                );
              }
            } else {
              ...
              if (!(<any>Object).values(Overwatch2Heroes).includes(opyHero)) {
                error("Unknown hero '" + opyHero + "'");
              }
              result[opyCategory][opyTeam][opyHero] = {};
              let heroValues =
                customGameSettingsSchema[opyCategory].values[
                  opyHero as Overwatch2Heroes
                ]?.values;
              ...
              Object.assign(
                ...
                decompileCustomGameSettingsDict(
                  ...
                  heroValues,
                  {
                    invalidButAcceptedProperties:
                      customGameSettingsSchema[opyCategory].values.general
                        ?.values ?? error("No general values for heroes"),
                  },
                ),
              );
            }
        ...
        if (dict.length > 0) {
          result[opyCategory][opyTeam].general =
            decompileCustomGameSettingsDict(
              dict,
              customGameSettingsSchema.heroes.values.general?.values ??
                error("No general values for heroes"),
            );
        }
      }
      ...
      while (i < workshopSettings.length) {
        ...
        if (nextColonIndex < 0) {
          error(
            "Expected a ':', but found none, while parsing workshop settings",
          );
        }
        ...
        if (nextNewlineIndex < 0) {
          error(
            "Expected a newline, but found none, while parsing workshop settings",
          );
        }
  ...
  for (var i = 0; i < varNames.length; i++) {
    ...
    if (i === 0) {
      ...
      if (varNames[i].search(/\s/) >= 0) {
        ...
        if (elems.length !== 2) {
          error(
            "Could not parse variables field: too many elements on '" +
              varNames[i] +
              "'",
          );
        }
        addVariable(
          translateNameToAvoidKeywords(
            elems[0],
            isInGlobalVars ? "globalvar" : "playervar",
          ),
          isInGlobalVars,
          currentVarIndex,
        );
