...
export function decompileRuleToAst(content: string): Ast {
  ...
  if (bracketPos.length !== 4) {
    error(
      "Invalid rule (mismatched brackets): has " +
        bracketPos.length +
        " top-level brackets, should be 4",
    );
  }
  ...
  for (var i = 0; i < bracketPos2.length - 2; i += 2) {
    var fieldName = topy(
      ruleContent.substring(bracketPos2[i] + 1, bracketPos2[i + 1]),
      ruleKw,
    );
    if (fieldName === "__event__") {
      eventInst = splitInstructions(
        ruleContent.substring(bracketPos2[i + 1] + 1, bracketPos2[i + 2]),
      );
...
export function decompileConditions(content: string) {
  var conditions = splitInstructions(
  ...
  for (let condition of conditions) {
    ...
    if (condition.startsWith(tows("__disabled__", ruleKw))) {
      ...
      condition = condition
        .substring(tows("__disabled__", ruleKw).length)
        .trim();
    }
...
export function decompile(content: string): Ast {
  ...
  for (var operatorGroup of wsOperators) {
    ...
    if (operatorCheck.operatorFound !== null) {
      ...
      var operands = [
        ...
        content.slice(
          operatorCheck.operatorPosition + operatorCheck.operatorFound.length,
        ),
      ];
      if (operator in binaryOpToFuncMapping) {
        return new Ast(
          binaryOpToFuncMapping[operator as keyof typeof binaryOpToFuncMapping],
          [decompile(operands[0]), decompile(operands[1])],
        );
      } else if (operator in modifyOpToFuncMapping) {
        return new Ast("__modifyVar__", [
          decompile(operands[0]),
          new Ast(
            modifyOpToFuncMapping[
              operator as keyof typeof modifyOpToFuncMapping
            ],
            [],
            [],
            "__Operation__",
          ),
          decompile(operands[1]),
        ]);
      } else if (operator === "?") {
        ...
        return new Ast("__ifThenElse__", [
          decompile(operands[0]),
          decompile(elseOperands[0]),
          decompile(elseOperands[1]),
        ]);
      } else {
    ...
    return new Ast("__valueInArray__", [
      ...
      decompile(
        content.substring(
          ...
          bracketPos[bracketPos.length - 1],
        ),
      ),
    ]);
  }
  ...
  if (dotOperands.length === 2) {
    ...
    if (isNumber(dotOperands[0])) {
      if (dotOperands[0].startsWith("-")) {
        return new Ast("__negate__", [
          getAstForNumber(Number(content.substring(1))),
        ]);
      } else {
    ...
    if (dotOperands[0] === tows("__global__", valueFuncKw)) {
      return new Ast("__globalVar__", [
        new Ast(
          translateVarToPy(dotOperands[1], true),
          [],
          [],
          "GlobalVariable",
        ),
      ...
      return new Ast("__playerVar__", [
        decompile(dotOperands[0]),
        new Ast(
          translateVarToPy(dotOperands[1], false),
          [],
          [],
          "PlayerVariable",
        ),
  ...
  if (bracketPos.length === 2) {
    ...
    error(
      "Mismatched top-level brackets in action " +
        content +
        ": expected 0 or 2, found " +
        bracketPos.length,
    );
  }
  ...
  if (
    bracketPos.length === 2 &&
    content.substring(bracketPos[0] + 1, bracketPos[1]).trim().length === 0
  ) {
    hasArgs = false;
  ...
  if (isNumber(name)) {
    if (name.startsWith("-")) {
      return new Ast("__negate__", [
        getAstForNumber(Number(name.substring(1))),
      ]);
    } else {
  ...
  if (name === "__modifyGlobalVariable__") {
    return new Ast("__modifyVar__", [
      new Ast("__globalVar__", [
        new Ast(translateVarToPy(args[0], true), [], [], "GlobalVariable"),
      ]),
      new Ast(
        topy(args[1], constantValues["__Operation__"]),
        [],
        [],
        "__Operation__",
      ),
  ...
  if (name === "__modifyGlobalVariableAtIndex__") {
    return new Ast("__modifyVar__", [
      ...
      new Ast(
        topy(args[2], constantValues["__Operation__"]),
        [],
        [],
        "__Operation__",
      ),
  ...
  if (name === "__modifyPlayerVariableAtIndex__") {
    return new Ast("__modifyVar__", [
      ...
      new Ast(
        topy(args[3], constantValues["__Operation__"]),
        [],
        [],
        "__Operation__",
      ),
  ...
  if (name === "__compare__") {
    ...
    return new Ast(funcToOpMapping[args[1] as keyof typeof funcToOpMapping], [
      decompile(args[0]),
      decompile(args[2]),
    ]);
  }
  ...
  if (name === "__round__") {
    return new Ast("__round__", [
      decompile(args[0]),
      new Ast(
        topy(args[1], constantValues.__Rounding__),
        [],
        [],
        "__Rounding__",
      ),
      ...
      if (args.length !== wsFuncKw[name].args?.length) {
        error(
          "Function '" +
            ...
            args.length +
            "args, expected " +
            funcKw[name].args?.length ?? "<ERR: UNDEFINED ARGS>",
        );
      }
    }
  }
  ...
  for (var i = 0; i < args.length; i++) {
    if (name !== "__array__") {
      if (wsFuncKw[name].args?.[i].type in constantValues) {
        astArgs.push(
          new Ast(
            args[i] === "__removed_from_ow2__"
              ? args[i]
              : topy(args[i], constantValues[wsFuncKw[name].args?.[i].type]),
            [],
            [],
            wsFuncKw[name].args?.[i].type,
          ),
        );
      } else if (wsFuncKw[name].args?.[i].type === "GlobalVariable") {
        astArgs.push(
          new Ast(translateVarToPy(args[i], true), [], [], "GlobalVariable"),
        );
      } else if (wsFuncKw[name].args?.[i].type === "PlayerVariable") {
        astArgs.push(
          new Ast(translateVarToPy(args[i], false), [], [], "PlayerVariable"),
        );
      } else if (wsFuncKw[name].args?.[i].type === "Subroutine") {
        astArgs.push(
          new Ast(translateSubroutineToPy(args[i]), [], [], "Subroutine"),
        );
      } else {
