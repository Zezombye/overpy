...
export function isNumber(x: unknown) {
  ...
  return (
    typeof x === "number" ||
    !isNaN(parseInt(x as string)) ||
    !isNaN(parseFloat(x as string))
  );
}
...
export function safeEval(script: string) {
  ...
  while (JSInterpreter.getStatus() === Interpreter.Status.STEP) {
    ...
    if (
      step % STACK_SIZE_CHECK_INTERVAL === 0 &&
      roughValueMemoryBytes(JSInterpreter.getStateStack()) >
        MAX_SCRIPT_STACK_SIZE
    ) {
  ...
  if (typeof scriptResult !== "string") {
    error(
      `JavaScript macro returned value with type of ${typeof scriptResult}, expected string`,
    );
  }
  return scriptResult;
}
