...
*/
export function isTypeSuitable(
  expectedType: Type | Type[],
  receivedType: Type | Type[],
  valueTypeIsSuitable = true,
): boolean {
  let receivedTypeName =
    typeof receivedType === "string"
      ? receivedType
      : Object.keys(receivedType)[0];

  if (receivedType instanceof Array) {
    //Check if each of the received type is valid for the expected type.
    return receivedType.every((x) =>
      isTypeSuitable(expectedType, x, valueTypeIsSuitable),
    );
  }

  if (expectedType instanceof Array) {
    //Check if the received type is valid for any of the expected types.
    return expectedType.some((x) =>
      isTypeSuitable(x, receivedType, valueTypeIsSuitable),
    );
  }
  ...
  if (typeof receivedType === "string") {
    if (typeof expectedType === "string") {
      ...
      if (receivedType === "Value" && valueTypeIsSuitable) {
        return (
          expectedType === "Value" ||
          expectedType === "Array" ||
          typeMatrix["Object"].includes(expectedType)
        );
      } else if (expectedType === "Value" && valueTypeIsSuitable) {
        return (
          receivedType === "Array" ||
          typeMatrix["Object"].includes(receivedType)
        );
      } else {
      ...
      if (expectedTypeName === "Array") {
        ...
        ["Vector", "Direction", "Position", "Velocity"].includes(
          expectedTypeName,
        )
      ) {
    ...
    if (receivedTypeName === "Array") {
      if (typeof expectedType === "string") {
        ...
        var expectedTypeName = Object.keys(expectedType)[0];
        if (expectedTypeName === "Array") {
          return isTypeSuitable(
            expectedType[expectedTypeName],
            receivedType[receivedTypeName],
          );
        } else if (
          ["Vector", "Direction", "Position", "Velocity"].includes(
            expectedTypeName,
      ...
      ["Vector", "Direction", "Position", "Velocity"].includes(receivedTypeName)
    ) {
      if (
        typeof expectedType === "string" &&
        typeof receivedType[receivedTypeName] !== "string"
      ) {
        //The default type for vectors is float.
        return (
          isTypeSuitable(expectedType, receivedTypeName) &&
          (receivedType[receivedTypeName] as Type[]).every((x) =>
            isTypeSuitable("float", x),
          )
        );
      } else if (typeof expectedType === "object") {
        var expectedTypeName = Object.keys(expectedType)[0];
        if (expectedTypeName === "Array") {
          ...
          if (isTypeSuitable(expectedTypeName, receivedTypeName)) {
            if (
              expectedType[expectedTypeName].length !==
              receivedType[receivedTypeName].length
            ) {
              return false;
            } else {
              for (var i = 0; i < expectedType[expectedTypeName].length; i++) {
                if (
                  !isTypeSuitable(
                    expectedType[expectedTypeName][i],
                    receivedType[receivedTypeName][i],
                  )
                ) {
                  return false;
                }
  ...
  error(
    "Unhandled expected type '" +
      JSON.stringify(expectedType) +
      "' or received type '" +
      JSON.stringify(receivedTypeName) +
      "'",
  );
}
...
export function parseType(tokens: Token[]): Ast {
  ...
  if (
    ...
    return new Ast(
      "Array",
      [parseType(tokens.slice(0, tokens.length - 2))],
      [],
      "Type",
    );
  }
  ...
  if (
    ...
    error(
      "Expected ']' at end of type, but got '" +
        tokens[tokens.length - 1].text +
        "'",
    );
  }
  ...
  if (
    ...
    error(
      "Expected 'int', 'float' or 'enum' before '[', but got '" +
        tokens[0].text +
        "'",
    );
  }
  ...
  if (tokens[0].text === "int" || tokens[0].text === "float") {
    ...
    return new Ast(
      tokens[0].text,
      [getAstForNumber(min), getAstForNumber(max)],
      [],
      "Type",
    );
