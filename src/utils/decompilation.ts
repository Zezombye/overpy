...
export function decompileCustomGameSettingsDict(
  ...
  for (var elem of dict) {
    ...
    var objKeys = Object.keys(kwObj)
      .sort(function (a, b) {
        var locA =
          currentLanguage in kwObj[a]
            ? kwObj[a][currentLanguage]
            : kwObj[a]["en-US"];
        var locB =
          currentLanguage in kwObj[b]
            ? kwObj[b][currentLanguage]
            : kwObj[b]["en-US"];
        return locA.localeCompare(locB);
      })
      .reverse();
    var invalidButAcceptedPropertiesObjKeys = Object.keys(
      options.invalidButAcceptedProperties,
    )
      .sort(function (a, b) {
        var locA =
          currentLanguage in options.invalidButAcceptedProperties[a]
            ? options.invalidButAcceptedProperties[a][currentLanguage]
            : options.invalidButAcceptedProperties[a]["en-US"];
        var locB =
          currentLanguage in options.invalidButAcceptedProperties[b]
            ? options.invalidButAcceptedProperties[b][currentLanguage]
            : options.invalidButAcceptedProperties[b]["en-US"];
        return locA.localeCompare(locB);
      })
      .reverse();
        ...
        if (currentLanguage in options.invalidButAcceptedProperties[key]) {
          if (
            elem
              .toLowerCase()
              .startsWith(
                options.invalidButAcceptedProperties[key][
                  currentLanguage
                ].toLowerCase(),
              )
          ) {
            keyName = key;
            value = elem.substring(
              options.invalidButAcceptedProperties[key][currentLanguage].length,
            );
          ...
          elem
            .toLowerCase()
            .startsWith(
              options.invalidButAcceptedProperties[key]["en-US"].toLowerCase(),
            )
        ) {
          keyName = key;
          value = elem.substring(
            options.invalidButAcceptedProperties[key]["en-US"].length,
          );
          isInvalidButAcceptedProperty = true;
          break;
        }
    ...
    if (typeof kwObj[keyName].values === "object") {
      ...
      ["__boolYesNo__", "__boolOnOff__", "__boolEnabled__"].includes(
        kwObj[keyName].values,
      )
    ) {
      ...
      [
        "__boolReverseYesNo__",
        "__boolReverseOnOff__",
        "__boolReverseEnabled__",
      ].includes(kwObj[keyName].values)
    ) {
      value = topy(value, customGameSettingsKw);
      if (["__yes__", "__enabled__", "__on__"].includes(value)) {
        value = false;
 ...
 */
export function splitStrOnDelimiter(
  content: string,
  delimiter: string,
  getAllMembers = true,
  rtl = false,
  ...
  for (var i = 0; i < content.length; i++) {
    ...
    if (
      bracketPosCheckIndex < bracketPos.length &&
      i >= bracketPos[bracketPosCheckIndex]
    ) {
      ...
      if (
        ...
        delimiter === "." &&
        (content.substring(0, i).toLowerCase().endsWith("créer une i") ||
          content.substring(0, i).toLowerCase().endsWith("créer une i.a") ||
          content.substring(0, i).toLowerCase().endsWith("est une i") ||
          content.substring(0, i).toLowerCase().endsWith("est une i.a") ||
          content.substring(0, i).toLowerCase().endsWith("détruire une i") ||
          content.substring(0, i).toLowerCase().endsWith("détruire une i.a") ||
          content
            .substring(0, i)
            .toLowerCase()
            .endsWith("forcer le nom de l’i") ||
          content
            .substring(0, i)
            .toLowerCase()
            .endsWith("forcer le nom de l’i.a") ||
          content
            .substring(0, i)
            .toLowerCase()
            .endsWith("arrêter de forcer le nom de l’i") ||
          content
            .substring(0, i)
            .toLowerCase()
            .endsWith("arrêter de forcer le nom de l’i.a") ||
          content
            .substring(0, i)
            .toLowerCase()
            .endsWith("détruire toutes les i") ||
          content
            .substring(0, i)
            .toLowerCase()
            .endsWith("détruire toutes les i.a"))
      ) {
        continue;
      }
      delimiterPos.push(i);
  ...
  for (var i = 0; i < delimiterPos.length - 1; i++) {
    var currentStr = content.substring(
      delimiterPos[i] + delimiter.length,
      delimiterPos[i + 1],
    );
  ...
  if (!getAllMembers && result.length > 2) {
    if (rtl) {
      result = [
        result.slice(0, result.length - 1).join(delimiter),
...
export function getOperatorInStr(
  content: string,
  operators: string[],
  rtlPrecedence = false,
) {
...
export function getBracketPositions(
  content: string,
  returnFirstPair = false,
  stringIncludesApos = false,
): number[] {
  var bracketsPos = [];
  var bracketsLevel = 0;
  var currentPositionIsString = false;
  ...
  for (var i = 0; i < content.length; i++) {
    if (
      !currentPositionIsString &&
      startsWithParenthesis(content.substring(i))
    ) {
      bracketsLevel++;
      if (bracketsLevel === 1) {
        bracketsPos.push(i);
      ...
      !currentPositionIsString &&
      (content.charAt(i) === '"' ||
        (content.charAt(i) === "'" && stringIncludesApos))
    ) {
      currentPositionIsString = !currentPositionIsString;
      currentStrDelimiter = content.charAt(i);
    } else if (content.charAt(i) === currentStrDelimiter) {
