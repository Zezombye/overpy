...
var datatoolPath =
  process.env.DATATOOL_PATH ?? "%USERPROFILE%\\toolchain-release\\DataTool.exe";
var overwatchPath =
  process.env.OVERWATCH_PATH ?? "C:\\Program Files (x86)\\Overwatch";
...
function getGuids() {
  ...
  for (let guidGlob of Object.entries(guids)) {
    ...
    enUSFuzzyToGuidMap.set(
      guidGlob[1]["enUS"].replace(/[\.,;'\s()-]/g, "").toLowerCase(),
      guid,
    );
  }
...
function addTranslations(content) {
  if (!("guid" in content) || content.guid === "<unknown guid>") {
    assert(
      Object.keys(content).includes("en-US"),
      "GUID-less content does not have an en-US key: " +
        JSON.stringify(content),
    );

    if (fuzzyMatch) {
      content.guid = enUSFuzzyToGuidMap.get(
        content["en-US"].replace(/[\.,;'\s()-]/g, "").toLowerCase(),
      );
    } else {
