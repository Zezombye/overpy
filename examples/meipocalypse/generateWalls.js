...
for (var wall of walls) {
    ...
    result += "\t@Condition currentMap == " + map + " and currentZone >= " + wall.minZone + " and currentZone <= " + wall.maxZone + " and eventPlayer.getPosition()." + wall.axis + " " + (wall.block === "up" ? ">" : "<") + " " + (wall.value + (wall.block === "up" ? -1 : +1)) + " and eventPlayer.getPosition()." + (wall.axis === "x" ? "z" : "x") + " > " + wall.start + " and eventPlayer.getPosition()." + (wall.axis === "x" ? "z" : "x") + " < " + wall.end + "\n";
    result += "\tdo:\n";
    ...
    for (var i = 0; i < nbBeams; i++) {
        result += "\tcreateBeam(getAllPlayers(), Beam.GRAPPLE, vect(" + (currentx + 0.01) + "," + starty + "," + currentz + "),vect(" + currentx + "," + endy + "," + currentz + "), Color.ORANGE, EffectReeval.VISIBILITY)\n";
        currentx += stepvectx;
        currentz += stepvectz;
    }
}
